...
/* Indonesian procurement categories */
const CATS = [
  'Alat Tulis Kantor',
  'Peralatan IT',
  'Laptop',
...
export default function Submit() {
  ...
  /* Auto-compute requested_amount from qty × unit price */
  function handleQtyPrice(k, v) {
    const u = { ...f, [k]: v }
    const q = parseFloat(u.quantity)
    const p = parseFloat(u.unit_price)
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      setError(
        Array.isArray(d)
          ? d.map(x => x.msg).join('; ')
          : (d || 'Submission failed. Please try again.')
      )
    } finally {
      setSubmitting(false)
    }
      ...
      <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <Section title="Procurement Details">
          <Field label="Procurement Title" required>
            <input
              className="v-input"
              placeholder="e.g. Pengadaan Laptop Dinas Q3 2025"
              value={f.title}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <Field label="Unit Price (Rp)" required>
              <input
                ...
                value={f.unit_price}
                onChange={e => handleQtyPrice('unit_price', e.target.value)}
                required
              />
            </Field>
            ...
            <Field
              ...
              required
              hint="Auto-calculated from qty × unit price"
            >
              <input
                type="number" min="0.01" step="any"
                className="v-input"
            ...
            <Field
              label="Reference / Market Price (Rp)"
              required
              hint="E-Katalog or market reference price"
            >
              <input
                type="number" min="0.01" step="any"
                className="v-input"
                placeholder="e.g. 7500000"
                value={f.reference_price}
        ...
        <Section title="Description & Supporting Information">
          ...
          <Field
            ...
            hint="Referensi SPSE, nomor NPWP, kode satker, dll."
          >
            <textarea
              className="v-textarea"
              rows={2}
              placeholder="e.g. Referensi: SPSE-1001-2025-0042 | Kode Satker: 10.01"
              value={f.supporting_info}
